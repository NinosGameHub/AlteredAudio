...
public:
    ...
    void setDrive(float d)           { drive  = juce::jlimit(1.0f, 100.0f, d); driveSm.setTargetValue(drive); }
    void setWetDry(float w)          { wetDry = juce::jlimit(0.0f, 1.0f,   w); wetDrySm.setTargetValue(wetDry); }
    void setBitDepth(int bits)       { bitDepth = juce::jlimit(1, 24, bits); }

    int getLatencySamples() const override { return (int)oversampling.getLatencyInSamples(); }

private:
    float processSample(float input, float driveAmt) const;
    ...
    // Smoothed parameters — drive runs at the oversampled rate
    juce::SmoothedValue<float> driveSm  { 1.0f };
    juce::SmoothedValue<float> wetDrySm { 1.0f };

    // 2x oversampling (factor = 1 means 2^1 = 2x)
    juce::dsp::Oversampling<float> oversampling{
        2, 1, juce::dsp::Oversampling<float>::filterHalfBandPolyphaseIIR };

    // Aligns the dry path with the oversampler's (fractional) group delay
    // so wet/dry blends don't comb-filter.
