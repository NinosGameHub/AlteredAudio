...
public:
    DelayPlugin() : SingleModuleProcessor(std::make_unique<DelayModule>(), createLayout())
    {
        ...
        p_spread    = apvts_.getRawParameterValue(ParamID::delaySpread);
    ...
    static APVTS::ParameterLayout createLayout()
    {
        ...
        g->addChild(std::make_unique<juce::AudioParameterFloat>(PID{ParamID::delayTime,      1}, "Time (ms)",     NR(0.0f, 2000.0f, 0.0f, 0.5f), 250.0f));
        g->addChild(std::make_unique<juce::AudioParameterFloat>(PID{ParamID::delayFeedback,  1}, "Feedback",      NR(0.0f,   0.99f),              0.3f));
        g->addChild(std::make_unique<juce::AudioParameterFloat>(PID{ParamID::delaySpread,    1}, "Spread R (ms)", NR(0.0f,  500.0f),              0.0f));
...
protected:
    void updateModuleParameters() override
    {
        ...
        mod->setDelayTimeMs(juce::jlimit(0.0f, 2000.0f, (float)*p_time));
        mod->setFeedback  (*p_feedback);
        mod->setSpreadMs  (*p_spread);
